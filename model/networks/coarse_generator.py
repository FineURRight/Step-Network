...
class CoarseBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, cated_stream2=False, cal_att=False):
        ...
        if cated_stream2:
            conv_block += [nn.Conv2d(dim*2, dim, kernel_size=3, padding=1, bias=use_bias, dilation=2),
                       norm_layer(dim),
                       nn.ReLU(True)]
        else:
            conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=use_bias, dilation=2),
                           norm_layer(dim),
                           nn.ReLU(True)]
        if use_dropout:
...
class CBModel(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=3, gpu_ids=[], padding_type='reflect', n_downsampling=2):
        ...
        for i in range(n_blocks):
            attBlock.append(CoarseBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, cated_stream2=cated_stream2[i]))
    ...
    def forward(self, input):
        ...
        # img1, pose1, pose2, par1 = input
        par2, mask = self.PB(torch.cat((par1, pose1, pose2), 1))
        # print(par2.shape)(1,8,256,256)
        x1=img1
        x2=torch.cat((pose1, pose2, par1, par2), 1)
        x3=torch.cat((par2,pose2), 1)
        # down_sample
