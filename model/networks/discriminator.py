...
class ResDiscriminator(BaseNetwork):
    ...
    """
    def __init__(self, opt, input_nc=3, ndf=64, img_f=1024, layers=6, norm='none', activation='LeakyReLU', use_spect=True,
                 use_coord=False):
...
class Style_Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(18, 64, kernel_size=4, stride=2, padding=1)
        ...
        self.conv5 = nn.Conv1d(512, 1, kernel_size=4, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        x=self.fc1(self.BN5(self.conv5(x)))
        x = self.sigmoid(self.BN6(x))
        return x.view(-1, 1)
