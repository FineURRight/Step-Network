...
def map_to_cord(pose_map, threshold=0.1):
    ...
    #y, x, z= np.where(pose_map > threshold)
...
def load_pose_cords(path):
    txt=open(path,'r')
    BP_cor_record=np.zeros((18,2))
    BP_cord_tmp=torch.zeros([256,256,18])
    for idx,line in enumerate(txt):
        cords=line.split(",")
        cords_x=int(cords[0])
        cords_y=int(cords[1])
        BP_cor_record[idx,0]=int(cords[0])
        BP_cor_record[idx,1]=int(cords[1])
        if(cords_x==-1):
            continue
        BP_cord_tmp[cords_x,cords_y+40,idx]=1
    BP_cord=BP_cord_tmp.numpy()
    return BP_cord,BP_cor_record
